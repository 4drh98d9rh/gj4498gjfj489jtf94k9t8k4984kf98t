...
async function checkDefaultPassword() {
    try {
        ...
        if (!data.is_default) {
            ...
            // Add message
            const container = display.closest('.bg-slate-800\\/40');
            if (container) {
                const msg = document.createElement('span');
                msg.className = 'text-xs text-amber-400 ml-2';
                msg.textContent = '(your custom password)';
...
async function updatePath(type) {
    ...
    try {
        ...
        // If dashboard path changed, reload after short delay
        if (type === 'dashboard') {
            setTimeout(() => {
