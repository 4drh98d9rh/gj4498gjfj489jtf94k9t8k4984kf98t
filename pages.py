    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-slate-900/60 border border-slate-800/70 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="relative w-16 h-16 shrink-0">
                ...
                <div class="absolute inset-0 flex items-center justify-center text-[10px] font-mono font-bold text-blue-400" id="ring-traffic-pct">0%</div>
            </div>
            <div class="text-center sm:text-right">
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Data Out</p>
                <p class="text-lg font-bold mt-0.5 text-slate-100 font-mono" id="ring-traffic-val">0.0 MB</p>
            </div>
        </div>
        <div class="bg-slate-900/60 border border-slate-800/70 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="relative w-16 h-16 shrink-0">
                ...
                <div class="absolute inset-0 flex items-center justify-center text-[10px] font-mono font-bold text-indigo-400" id="ring-conns-pct">0/0</div>
            </div>
            <div class="text-center sm:text-right">
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Inbounds</p>
                <p class="text-lg font-bold mt-0.5 text-slate-100 font-mono" id="ring-conns-val">0 active</p>
            </div>
        </div>
        <div class="bg-slate-900/60 border border-slate-800/70 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="relative w-16 h-16 shrink-0">
                ...
                <div class="absolute inset-0 flex items-center justify-center text-[10px] font-mono font-bold text-amber-400" id="ring-cpu-pct">0%</div>
            </div>
            <div class="text-center sm:text-right">
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">CPU Thread</p>
                <p class="text-lg font-bold mt-0.5 text-slate-100 font-mono" id="ring-cpu-val">0.0%</p>
            </div>
        </div>
        <div class="bg-slate-900/60 border border-slate-800/70 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="relative w-16 h-16 shrink-0">
                ...
                <div class="absolute inset-0 flex items-center justify-center text-[10px] font-mono font-bold text-rose-400" id="ring-ram-pct">0%</div>
            </div>
            <div class="text-center sm:text-right">
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">RAM Pool</p>
                <p class="text-lg font-bold mt-0.5 text-slate-100 font-mono" id="ring-ram-val">0 MB</p>
            </div>
        </div>
    </div>
...
async function updateStats() {
    try {
        ...
        // Percentage: assume a max of 1000 MB for the ring (100%)
        const trafficPct = Math.min(100, Math.round((totalMB / 1000) * 100));
        document.getElementById('ring-traffic-pct').textContent = trafficPct + '%';
        // Update circle stroke-dasharray
        const trafficCircle = document.querySelector('.text-blue-500.circle-chart');
        if (trafficCircle) {
            const dash = Math.min(100, trafficPct);
            trafficCircle.setAttribute('stroke-dasharray', dash + ', 100');
        }
        // Inbounds ring
        const activeLinks = d.active_links || 0;
        const totalLinks = d.links_count || 0;
        document.getElementById('ring-conns-val').textContent = activeLinks + ' active';
        document.getElementById('ring-conns-pct').textContent = activeLinks + '/' + totalLinks;
        const connsPct = totalLinks ? Math.min(100, Math.round((activeLinks / totalLinks) * 100)) : 0;
        const connsCircle = document.querySelector('.text-indigo-500.circle-chart');
        if (connsCircle) {
            connsCircle.setAttribute('stroke-dasharray', connsPct + ', 100');
        }
        // CPU ring
        const cpu = d.cpu_percent || 0;
        document.getElementById('ring-cpu-val').textContent = cpu.toFixed(1) + '%';
        document.getElementById('ring-cpu-pct').textContent = Math.round(cpu) + '%';
        const cpuCircle = document.querySelector('.text-amber-500.circle-chart');
        if (cpuCircle) {
            cpuCircle.setAttribute('stroke-dasharray', Math.round(cpu) + ', 100');
        }
        // RAM ring
        const ramPct = d.ram_percent || 0;
        const ramUsed = d.ram_used_mb || 0;
        document.getElementById('ring-ram-val').textContent = ramUsed + ' MB';
        document.getElementById('ring-ram-pct').textContent = Math.round(ramPct) + '%';
        const ramCircle = document.querySelector('.text-rose-500.circle-chart');
        if (ramCircle) {
            ramCircle.setAttribute('stroke-dasharray', Math.round(ramPct) + ', 100');
        }
