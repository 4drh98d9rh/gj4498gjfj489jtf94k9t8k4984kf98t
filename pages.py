...
# All HTML templates: login, dashboard, subscription info page, and new public user page.
...
async function loadConfigs() {
    try {
        ...
        container.innerHTML = links.map(l => {
            ...
            // Determine speed unit display
            let speedDisplay = '∞';
            if (l.speed_limit_bytes && l.speed_limit_bytes > 0) {
                // Convert to Mbps, KB/s, or MB/s based on the stored unit? We'll just show Mbps for simplicity here.
...
# ── NEW: Public subscription page for end users ─────────────────────────────
